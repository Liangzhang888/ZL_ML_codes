...
def trainer_singlemodal(model, train_dataloader, optimizer, epochs, batch_size, logger, test_dataloader, device, save_path, loss_fn, decision_stragety = 'single'):
    ...
    for epoch in range(epochs):
        ...
        for batch_idx, (batch) in enumerate(train_dataloader):
            ...
            eeg_data = batch['eeg_data'].to(torch.float32).unsqueeze(1).to(device)
            topoembedding = batch['topoembedding'].to(torch.float32).unsqueeze(1).to(device)
            label = batch['label_cls'].to(torch.float32).to(device)
            if decision_stragety == 'single':
                output_cls = model(x= eeg_data)
            elif decision_stragety == 'SFL':
                output_cls = model(x = topoembedding)
            # output_cls = model(x= topoembedding)
        ...
        if (epoch+1) % 5==0: # 每5个epoch 存储一遍数据
            ...
            spike_acc, spike_presion, spike_recall, spike_f1, auc_val, loss_val = tester_singlemodal(test_dataloader,model, device, decision_stragety=decision_stragety)
...
def tester_singlemodal(test_dataloader,model, device, decision_stragety = 'single'):
    ...
    with torch.no_grad():
        for batch in  test_dataloader:
            ...
            if decision_stragety == 'single':
                output_cls = model(x= eeg_data.unsqueeze(1)).cpu()
            elif decision_stragety == 'SFL':
                output_cls = model(x = topoembedding.unsqueeze(1)).cpu()
